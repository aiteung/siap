...
func GetLoginInfofromPhoneNumber(phonenumber string, usertables []whatsauth.LoginInfo, db *sql.DB) (response whatsauth.LoginInfo) {
...
func GetHashPasswordfromUsername(username string, usertables []whatsauth.LoginInfo, db *sql.DB) (hashpassword string) {
	for _, table := range usertables {
		q := "select %s from %s where %s = '%s'"
...
func GetUserIdfromUsername(username string, usertables []whatsauth.LoginInfo, db *sql.DB) (userid string) {
	for _, table := range usertables {
		q := "select %s from %s where %s = '%s'"
